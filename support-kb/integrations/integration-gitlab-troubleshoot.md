---
id: integration-gitlab-troubleshoot
title: GitLab integration has stopped working
category: integrations
tags: [gitlab, integration, troubleshooting]
audience: customer
severity: normal
---

# GitLab integration has stopped working
...
In order of likelihood: the authorising user lost access in GitLab or left the
company, the OAuth grant was revoked by an administrator, the token expired
after a password change, or a permission scope changed on the GitLab side.

## Resolution

1. Open Settings > Integrations > GitLab and read the error on the card - it
   names the failing scope where GitLab tells us.
2. Choose **Reconnect** and re-authorise. This fixes expired and revoked tokens,
   which is the large majority of cases.
3. Confirm the authorising account still exists and retains access in GitLab.
   Prefer a service account over a person's account so staff turnover cannot
   break the integration.
4. Check GitLab's own status page - an outage there presents identically.
