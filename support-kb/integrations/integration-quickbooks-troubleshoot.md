---
id: integration-quickbooks-troubleshoot
title: QuickBooks integration has stopped working
category: integrations
tags: [integration, quickbooks, troubleshooting]
audience: customer
severity: normal
---

# QuickBooks integration has stopped working
...
In order of likelihood: the authorising user lost access in QuickBooks or left the
company, the OAuth grant was revoked by an administrator, the token expired
after a password change, or a permission scope changed on the QuickBooks side.

## Resolution

1. Open Settings > Integrations > QuickBooks and read the error on the card - it
   names the failing scope where QuickBooks tells us.
2. Choose **Reconnect** and re-authorise. This fixes expired and revoked tokens,
   which is the large majority of cases.
3. Confirm the authorising account still exists and retains access in QuickBooks.
   Prefer a service account over a person's account so staff turnover cannot
   break the integration.
4. Check QuickBooks's own status page - an outage there presents identically.
