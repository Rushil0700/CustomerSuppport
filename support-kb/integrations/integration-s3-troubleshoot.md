---
id: integration-s3-troubleshoot
title: Amazon S3 integration has stopped working
category: integrations
tags: [integration, s3, troubleshooting]
audience: customer
severity: normal
---

# Amazon S3 integration has stopped working
...
In order of likelihood: the authorising user lost access in Amazon S3 or left the
company, the OAuth grant was revoked by an administrator, the token expired
after a password change, or a permission scope changed on the Amazon S3 side.

## Resolution

1. Open Settings > Integrations > Amazon S3 and read the error on the card - it
   names the failing scope where Amazon S3 tells us.
2. Choose **Reconnect** and re-authorise. This fixes expired and revoked tokens,
   which is the large majority of cases.
3. Confirm the authorising account still exists and retains access in Amazon S3.
   Prefer a service account over a person's account so staff turnover cannot
   break the integration.
4. Check Amazon S3's own status page - an outage there presents identically.
